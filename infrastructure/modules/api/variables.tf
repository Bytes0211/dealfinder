...
variable "lambda_security_group_id" {
  description = "Security group ID from the pipeline module to reuse for API Lambda"
  type        = string
}
...
variable "lambda_memory_mb" {
  description = "Memory allocation for the API Lambda in MB"
  type        = number
  default     = 512
}

variable "lambda_timeout_seconds" {
  description = "Timeout for the API Lambda in seconds"
  type        = number
  default     = 30
}
